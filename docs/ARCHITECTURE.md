...
[User Browser] -> [Cloudflare Edge (React App)] -> [API Worker (Hono)]
                                                         |
                                                         v
                                              [Cloudflare API (SSL for SaaS)]
                                                         ^
                                                         |
                                              [Consumer Worker (Sync Status)]
                                                         ^
                                                         |
                                                   [Cron Worker]
...
2.  **API Worker:** 
    *   Validates plan limits (Free/Pro/Agency).
...
3.  **UI Feedback:** Returns the required **CNAME target** (the SaaS Fallback Origin, e.g., `custom.dcv.pcnaid.com`) to the user.

### 2. Verification & Issuance (Delegated)
1.  **User Action:** User creates a CNAME record in their DNS: `app.client.com` -> `custom.dcv.pcnaid.com`.
