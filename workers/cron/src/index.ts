...
export default {
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    // 1. Find domains that need status checking:
    //    - Domains not active yet (stuck in pending/issuing)
    //    - Active domains that haven't been updated in > 24 hours (for expiration updates)
  ...
  console.log(`Cron: Processing ${domains.length} domains (pending/issuing + active >24h old)`);
